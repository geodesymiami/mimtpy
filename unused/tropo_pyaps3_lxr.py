...
def create_parser():
    ...
    parser.add_argument('--download_only', action='store_true', default=False, help='whether delete data.')
    parser.add_argument('--SAFE_list', dest='safe_list', nargs=1, help='list of Sentinel safe files')
...
def cmd_line_parse(iargs=None):
    ...
    # without timeseries file and download_only function is closed
    elif any(not vars(inps)[key] for key in key_list) and inps.download_only == False:
        msg = 'ERROR: No input timeseries file, all the following options are required: \n{}'.format(key_list)
        msg += '\n\n'+EXAMPLE
        raise SystemExit(msg)
    
    # with download_only function opening
    elif inps.download_only:
        print('Weather data will be downloaded based on safe_list file. And only download function will operate.')
...
def main(iargs=None):
    ...
    # download only
    if inps.download_only:
        inps = check_inputs_download_only(inps)
        dload_grib_files(inps.grib_file_list,inps.trop_model)
