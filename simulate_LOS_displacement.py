...
def geocode(disp_data,disp_name,disp_sim,outdir):
    ...
    xmin = disp_sim["ul_lat"] + disp_sim["lat_step"] * disp_sim["rows"]
    xmax = disp_sim["ul_lat"]
    ymin = disp_sim["ul_lon"]
    ymax = disp_sim["ul_lon"] + disp_sim["lon_step"] * disp_sim["samples"]
    nrows = disp_sim["rows"]
    ncols = disp_sim["samples"]
    xres = disp_sim["lat_step"]
    yres = disp_sim["lon_step"]
    # output
...
def main(iargs=None):
    ...
    for file in path_list:
        if os.path.splitext(file)[0].split("_")[-1] == 'subtract':
    ...
    # write
    disp_name = "_".join(disp_sim_name[0].split('_')[:-2]) + '_LOS'
