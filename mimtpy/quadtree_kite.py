...
EXAMPLE = """example:
  ...
  And export points_disp.json/points_angle.json for grid_search 
...
def export_csv(sc, geometry, outfile):
    ...
    ll_coor = []
    point_disp = np.zeros((1,3))
    point_angle = np.zeros((1,4))    
    ...
    with open(filename, mode='w') as f:
        ...
        for lf in qt.leaves:
            ...
            disp_tmp = np.array([lat,lon,lf.mean]).reshape(1,3)
            point_disp = np.append(point_disp, disp_tmp, axis=0)
            angle_tmp = np.array([lat,lon,angle[0],angle[1]]).reshape(1,4)
            point_angle = np.append(point_angle, angle_tmp, axis=0)
    ...
    return point_disp[1:,:], point_angle[1:,:]
