...
EXAMPLE = """example:
    ...
    generate_script_RELAX_V2.py --sample_file /data/lvxr/MODEL/RELAX/Relax/examples/Iran/iran_vs_17251725.sh --parameters_number 2 --parameters_symbol V1,V2 --parameters_range 17,0.25,18,18,0.25,19 --template_dir /data/lvxr/MODEL/RELAX/Relax/examples/Iran/no_PBS_512/ --SLURM
"""
...
def create_parser():
    ...
    parser.add_argument('--SLURM', action='store_true', default=False, help='whether generate SLURM scripts. \n')
...
def change_text(inps, text_file, template_dir, para_dict,parameters):
    ...
    if inps.SLURM:
        line_numm = 0
        for line in lines:
            if line.find('#SBATCH -J') >= 0:
                lines[line_numm] = '#SBATCH -J' + text_file_change + '\n'
            if line.find('#SBATCH -o') >= 0:
                lines[line_numm] = '#SBATCH -o /scratch/09401/xlv/' + '"' + text_file_change + '-%j.out"' + '\n'
            if line.find('#SBATCH -e') >= 0:
                lines[line_numm] = '#SBATCH -e /scratch/09401/xlv/' + '"' + text_file_change + '-%j.err"' + '\n' 
            line_numm += 1
