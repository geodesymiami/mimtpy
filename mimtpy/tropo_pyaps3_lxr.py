...
def check_subset_grib_file(grib_files):
    ...
    flag_lat = []
    flag_lon = []
    number  = lat_grib.shape[0]
    for i in np.arange(number):
        if lat_grib[i,0] <= lat_grib_min and lat_grib[i,1] >= lat_grib_max:
            flag_lat.append(i)

    for j in np.arange(number):
        if lon_grib[j,0] <= lon_grib_min and lon_grib[j,1] >= lon_grib_max:
            flag_lon.append(j)
    grib_files_change = []
    grib_files_nochange = grib_files
    
    if list(set(flag_lat).intersection(set(flag_lon))) != None:
        common_prefix = list(set(flag_lat).intersection(set(flag_lon)))
        grib_subsets = list(grib_exist_uni[common_prefix])
        for grib_subset in grib_subsets:
            # whether they are for the same date
            positions = np.where(np.array(grib_exist_prefix) == grib_subset)[0]
    ...
    def sort_keys(s):
        c = re.findall(r'[A-Za-z]+|[\d.]+', os.path.split(s)[1])
        #c = re.findall(r'(\D)_(\d+)_(\d+)', s)
        return int(c[10])
    ...
    grib_files_final = strsort(grib_files_final)

    return grib_files_final
