...
def concatenate_patches(project, patches_name, network, datatype, refpoi_list):
    ...
    if pro_num == 2:
        ...
        scp_args = [patches_dir[0] + '/' + network + '/rdr_' + datatype + '_msk.h5', patches_dirs[1] + '/' + network + '/rdr_' + datatype + '_msk.h5', '--geo_file1', patches_dir[0] + '/' + network + '/rdr_geometry_msk.h5', '--geo_file2', patches_dir[1] + '/' + network + '/rdr_geometry_msk.h5', '--ref', str(refpoi_list[0, 0]), str(refpoi_list[0, 1]), '--output', temp_name,  '--outdir', outputdir + '/']
        scp_args = mu.separate_string_by_space(scp_args)
        ...
        for i in np.arange(pro_num - 1):
            ...
            if i == 0:
                ...
                scp_args = [patches_dir[0] + '/' + network + '/rdr_' + datatype + '_msk.h5', patches_dir[1] + '/' + network + '/rdr_' + datatype + '_msk.h5', '--geo_file1', patches_dir[0] + '/' + network + '/rdr_geometry_msk.h5', '--geo_file2', patches_dir[1] + '/' + network + '/rdr_geometry_msk.h5', '--ref', str(temp_refpoi[0]), str(temp_refpoi[1]), '--output', temp_name,  '--outdir', output_dir + '/']
                
            elif i > 0 and i < (pro_num - 1 - 1):
                temp_name = temp_files[i-1] + '_' + patch_range[i+1]
                temp_geo = temp_geos[i-1] + '_' + patch_range[i+1]
                scp_args = [output_dir + '/' + temp_files[i-1] +'.h5', patches_dir[i+1] + '/' + network + '/rdr_' + datatype + '_msk.h5', '--geo_file1', output_dir + '/' + temp_geos[i-1] + '.h5', '--geo_file2', patches_dir[i+1] + '/' + network + '/rdr_geometry_msk.h5', '--ref', str(temp_refpoi[0]), str(temp_refpoi[1]), '--output', temp_name, '--outdir', output_dir + '/']
            else:
...
def main(iargs=None):
    ...
    concatenate_patches(project, patches_name, network, datatype, refpoi_list)
