...
class TimeSeriesAnalysis:
            ...
            if sname == 'correct_LOD':
                ...
                if method:
                    if method == 'height_correlation':
                        fname1 = '{}_tropHgt_SET.h5'.format(os.path.splitext(fname0)[0]) #lxr

                    elif method == 'gacos':
                        fname1 = '{}_GACOS_SET.h5'.format(os.path.splitext(fname0)[0])  #lxr

                    elif method == 'pyaps':
                        fname1 = '{}_{}_SET.h5'.format(os.path.splitext(fname0)[0], model)  #lxr
    ...
    def run_tropospheric_delay_correction(self, step_name):
        ...
        fnames = self.get_timeseries_filename(self.template)[step_name]
        in_file = fnames['input']
        #out_file = fname['output']
        out_file_split = os.path.splitext(fnames['output'])[0].split('_')
        out_file = out_file_split[0] + '_' + out_file_split[1] + '.h5'
        if in_file != out_file:
        ...
        SET_in_file = 'timeseries_ERA5.h5'
        SET_out_file = 'timeseries_ERA5_SET.h5'
        
        iargs = [SET_in_file, '-g', geom_file]
        print('Solid earth tides correction')
        SET_file = './inputs/SET.h5'
        if os.path.isfile(SET_file) and get_dataset_size(SET_file) == get_dataset_size(SET_in_file):
            iargs = [SET_in_file, SET_file, '-o', SET_out_file, '--force']
            print('--------------------------------------------------')
            print('Use existed solid earth tides file: {}'.format(SET_file))
            print('\ndiff.py', ' '.join(iargs))
            mintpy.diff.main(iargs)
        else:
            cmd = 'solid_earth_tide.py '+' '.join(iargs)
            print(cmd)
            status = subprocess.Popen(cmd, shell=True).wait()
        return  
