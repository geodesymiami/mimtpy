...
def create_parser():
    ...
    parser.add_argument('--delta_lat', type=float, nargs=1, help='delta latitude added/subtracted from the maxima/minima lat')

    parser.add_argument('--delta_lon', nargs=1, type=float, help='delta longitude added/subtracted from the maxima/minima lon')
...
def process_kml(kml_file, delta_lat, delta_lon):
    ...
    print('the lat_min, lat_max, lon_min, lon_max are {}, {}, {}, {}'.format(lat_min2, lat_max2, lon_min2, lon_max2))
######################################################################################
def main(iargs=None):
    ...
    process_kml(kml_file, delta_lat, delta_lon)
######################################################################################
