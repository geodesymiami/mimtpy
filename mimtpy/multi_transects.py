...
*****************************************************************************************
   ...
   Usage:
       ...
       -R Ratio of profiles length between profile of north fault and profile of south fault
...
def get_start_end_point(Xf0, Yf0, Xf1, Yf1, L, dx, dy, ratio):
    ...
    if m != 0:
        ...
        xe = -ratio * L*np.sin(a)+Xf0
        ye = -ratio * L*np.cos(a)+Yf0
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(
            argv, "f:v:T:C:s:e:n:d:g:l:h:r:L:F:p:u:G:S:i:I:A:U:E:D:W:x:X:R:B:")
    ...
    try:
        ...
        dset, atr = readfile.read(velocityFile)
    except:
        usage()
        sys.exit(1)
    #k = list(h5file.keys())
    ...
    dx = float(atr['X_STEP'])*6375000.0*np.pi/180.0
    dy = float(atr['Y_STEP'])*6375000.0*np.pi/180.0
##############################################################################
    try:
        velocityFile2
    except:
        velocityFile2 = 'NoFile2'
        print(velocityFile2)
    if velocityFile2 and velocityFile2 != 'NoFile2':
        dset2, atr2 = readfile.read(velocityFile2)
        z2 = dset2[0:dset.shape[0], 0:dset.shape[1]]
...
##############################################################################
    ...
    if topography and topography != 'NoDEM':
        dem = readfile.read(topography, datasetName='height')[0] 
        dem_z = dem[0:dset.shape[0], 0:dset.shape[1]]
...
###############################################################################
    try:
        ...
        coherence = 'NoCoh'
        print(coherence)
    if coherence and coherence != 'NoCoh':
        coh = readfile.read(coherence)[0]
        coh_z = coh[0:dset.shape[0], 0:dset.shape[1]]
...
###############################################################################
    ...
    # read other faults file that want to be plot
    #Fault_lon1, Fault_lat1 = read_fault_coords(Ffile1, Dp)
    #Fault_lon2, Fault_lat2 = read_fault_coords(Ffile2, Dp)
    ...
    for Np in range(Num_profiles):
        ...
        y0, x0, y1, x1 = get_start_end_point(
            Xf0, Yf0, Xf1, Yf1, profile_Length, dx, dy, ratio)
        ...
        zi = z[y.astype(np.int), x.astype(np.int)]
        if velocityFile2 and velocityFile2 != 'NoFile2':
            zi2 = z2[y.astype(np.int), x.astype(np.int)]
        if topography and topography != 'NoDEM':
            dem_zi = dem_z[y.astype(np.int), x.astype(np.int)]
        if coherence and coherence != 'NoCoh':
            coh_zi = coh_z[y.astype(np.int), x.astype(np.int)]
        ...
        transect[:, 0] = zi
        if velocityFile2 and velocityFile2 != 'NoFile2':
            transect2 = np.zeros([len(D), ntrans])
            transect2[:, 0] = zi2
        if topography and topography != 'NoDEM':
            dem_transect = np.zeros([len(D), ntrans])
            dem_transect[:, 0] = dem_zi
        if coherence and coherence != 'NoCoh':
            coh_transect = np.zeros([len(D), ntrans])
            coh_transect[:, 0] = coh_zi
        ...
        if ntrans > 1:
            ...
            if lat_transect == 'Nan':
                for i in range(1, ntrans):
                    ...
                    transect[:, i] = zi
                    if velocityFile2 and velocityFile2 != 'NoFile2':
                        zi2 = get_transect(z2, X0, Y0, X1, Y1)
                        transect2[:, i] = zi2
                    if topography and topography != 'NoDEM':
                        dem_zi = get_transect(dem_z, X0, Y0, X1, Y1)
                        dem_transect[:, i] = dem_zi
                    if coherence and coherence != 'NoCoh':
                        coh_zi = get_transect(coh_z, X0, Y0, X1, Y1)
                        coh_transect[:, i] = con_zi
        ...
        if flip_profile == 'yes':
            ...
            if velocityFile2 and velocityFile2 != 'NoFile2':
                transect2 = np.flipud(transect2)
            if topography and topography != 'NoDEM':
                dem_transect = np.flipud(dem_transect)
            if coherence and coherence != 'NoCoh':
                coh_transect = np.flipud(coh_transect)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%                       
        ...
        if flip_updown == 'yes' and gpsFile != 'Nogps':
            ...
            transect = -1*transect
            if velocityFile2 and velocityFile2 != 'NoFile2':
                transect2 = -1 * transect2
            if topography and topography != 'NoDEM':
                dem_transect = dem_transect
            if coherence and coherence != 'NoCoh':
                coh_transect = coh_transect
        ...
        if velocityFile2 and velocityFile2 != 'NoFile2':
            avgInSAR2 = np.array(nanmean(transect2, axis=1))
            stdInSAR2 = np.array(nanstd(transect2, axis=1))
        ...
        if display_InSAR in ['on', 'On', 'ON']:
            ax.plot(D/1000.0, transect*100, 'o',
                    ms=1, mfc='Black', linewidth='0')
            if velocityFile2 and velocityFile2 != 'NoFile2':
                ax.plot(D/1000.0, transect2*100, 'o',
                    ms=1, mfc='Black', linewidth='0')
        ############################################################################
        ...
        if save_to_mat in ['ON', 'on', 'On']:
            ...
            dataset['fault_position'] = df0_km
            print('*****************************************')
            print('')
            print('writing transect to >>> '+matFile)
            sio.savemat(matFile, dataset, long_field_names=True)
            print('')
            print('*****************************************')
        ...
        if display_Standard_deviation in ['on', 'On', 'ON']:
            ...
            for i in np.arange(0.0, 1.01, 0.01):
                ...
                ax.plot(D/1000.0, (avgInSAR-i*stdInSAR) *
                        100, '-', color='#DCDCDC', alpha=0.5)
            for i in np.arange(0.0, 1.01, 0.01):
                ax.plot(D/1000.0, (avgInSAR+i*stdInSAR)*100, '-',
                        color='#DCDCDC', alpha=0.5)  # 'LightGrey')
            if velocityFile2 and velocityFile2 != 'NoFile2':
                for i in np.arange(0.0, 1.01, 0.01):
                    ax.plot(D/1000.0, (avgInSAR2-i*stdInSAR2) *
                            100, '-', color='#DCDCDC', alpha=0.5)
                for i in np.arange(0.0, 1.01, 0.01):
                    ax.plot(D/1000.0, (avgInSAR2+i*stdInSAR2)*100, '-',
                            color='#DCDCDC', alpha=0.5)  # 'LightGrey')
        #############################################################################
        if display_Average in ['on', 'On', 'ON']:
            ax.plot(D/1000.0, avgInSAR*100, 'r-', label='horizontal')
            if velocityFile2 and velocityFile2 != 'NoFile2':
                ax.plot(D/1000.0, avgInSAR2*100, 'b-', label='vertical')
                ax.legend(loc=1)
        ##############################################################################
        if topography and topography != 'NoDEM':
            dem_average = np.array(nanmean(dem_transect, axis=1))
            ax2 = ax.twinx()
            if coherence and coherence != 'NoCoh':
                dem_normalize = (dem_average - np.nanmin(dem_average)) / (np.nanmax(dem_average) - np.nanmin(dem_average))
                ax2.bar(D/1000.0, dem_normalize)
            else:
                ax2.plot(D/1000.0, dem_average, 'b-')
        ##############################################################################
        if coherence and coherence != 'NoCoh':
            coh_average = np.array(nanmean(coh_transect, axis=1))
            ax3 = ax.twinx()
            ax3.plot(D/1000.0, coh_average, 'm-')
        ##############################################################################
        try:
        ...
        font1 = {'family' : 'serif',
             'weight': 'normal',
             'size' : 18.}
        if velocityFile.find('velocity') != -1:
            ax.set_ylabel('LOS velocity [cm/yr]', font1)
        elif velocityFile.find('displacement') != -1:
            ax.set_ylabel('LOS displacement [cm]', font1)
        ax.set_xlabel('Distance along profile [km]', font1)
        labels = ax.get_xticklabels() + ax.get_yticklabels()
        [label.set_fontname('serif') for label in labels]
        ax.tick_params(which='both', direction='in', labelsize=18, bottom=True, top=True, left=True, right=False)
        ###################################################################
        ...
        ###################################################################
        if coherence and coherence != 'NoCoh':
            #ax3.set_ylabel('Coherence', font1)
            labels = ax3.get_yticklabels()
            [label.set_fontname('serif') for label in labels]
            ax3.tick_params(which='both', direction='in', labelsize=18, bottom=True, top=True, left=False, right=True)
        ...
        try:
            ...
            lbound = np.nanmin(transect)*100
            hbound = np.nanmax(transect)*100
        ...
        if topography and topography != 'NoDEM':
            if coherence and coherence != 'NoCoh':
                ...
                ax2.set_ylim(np.nanmin(dem_average), np.nanmax(dem_average) + 5000)
