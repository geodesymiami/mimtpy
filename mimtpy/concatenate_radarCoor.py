...
######################################################################################
NOTE = """
    Used to concatenate single type file except S1 file
"""

EXAMPLE = """example:

    concatenate_radarCoor.py miaplpy_NE/velocity_msk.h5  miaplpy_NNE/velocity_msk.h5 --geo-file1 miaplpy_NE/inputs/geometryRadar.h5 --geo-file2 miaplpy_NNE/intpus/geometryRadar.h5 --geo-full ./inputs/geometryRadar.h5 --geo-write --outdir miaplpyBig

    concatenate_radaCoor.py miaplpy_NE/velocity_msk.h5  miaplpy_NNE/velocity_msk.h5 --geo-full ./inputs/geometryRadar.h5 --outdir miaplpyBig
    
    concatenate_radarCoor.py miaplpy_NE/timeseries_msk.h5 miaplpy_NNE/timeseries_msk.h5 --geo-file1 miaplpy_NE/inputs/geometryRadar.h5 --geo-file2 miaplpy_NNE/inputs/geometryRadar.h5 --geo-full ./inputs/geometryRadar.h5  --out-suffix NE_NNE --outdir ./miaplpyBig/

    concatenate_radarCoor.py miaplpy_NE/maskPS.h5  miaplpy_NNE/maskPS.h5 --geo-file1 miaplpy_NE/inputs/geometryRadar.h5 --geo-file2 miaplpy_NNE/intpus/geometryRadar.h5 --geo-full ./inputs/geometryRadar.h5  --out-suffix NE_NNE --outdir miaplpyBig

    concatenate_radarCoor.py miaplpy_NE/maskTempCoh.h5  miaplpy_NNE/maskTempCoh.h5 --geo-file1 miaplpy_NE/inputs/geometryRadar.h5 --geo-file2 miaplpy_NNE/intpus/geometryRadar.h5 --geo-full ./inputs/geometryRadar.h5  --out-suffix NE_NNE --outdir miaplpyBig
"""

def create_parser():
    parser = argparse.ArgumentParser(description='Concatenate miaplpy results under radar coordinate',
...
def concatenate_ts(inps, lat1_flatten, lon1_flatten, lat2_flatten, lon2_flatten, unflatten_trans1, unflatten_trans2, rc1, rc2, ref_flag):
    ...
    ts_ref_atr = readfile.read_attribute(data_ref)
    ts_ref_open = h5py.File(data_ref,'r')
    ts_ref = ts_ref_open['/timeseries']
    print('Read the affiliate dataset') 
    ts_aff_atr = readfile.read_attribute(data_aff)
    ts_aff_open = h5py.File(data_aff,'r')
    ts_aff = ts_aff_open['/timeseries']
...
def write_vel(vel_joined, vel_atr, datatype, inps):
    ...
    atr_vel['REF_X'] = vel_atr['REF_X']
    atr_vel['REF_Y'] = vel_atr['REF_Y']
...
def write_ts(ts_joined_dataset, ts_atr, date_final, datatype, inps):
    ...
    atr_ts['REF_X'] = ts_atr['REF_X']
    atr_ts['REF_Y'] = ts_atr['REF_Y']
